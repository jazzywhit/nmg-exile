...
FNC_A3_LOG_RAND_VAR = compileFinal "
	'ARMA_LOG' callExtension format['A3_RANDOMVAR_LOG:%1 (v0011A)',_this];
	diag_log ('<infiSTAR.de>RandomVarLog| '+_this+' (v0011A)');
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['A','R','e','P','z','Y','c','3','x','P','L','y','b','F','Q','B','p','4','x','T','X','O','i','x','e','Y','X','h','b','S'];
...
FNC_A3_HACKLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_HACKLOG:%1 [FPS: %2|THREADS: %3] (v0011A)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>HL| '+_this+' (v0011A)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_HACKLOG'] call MAR_fnc_log;};
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_SURVEILLANCELOG:%1 [FPS: %2|THREADS: %3] (v0011A)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>SL| '+_this+' (v0011A)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_SURVEILLANCELOG'] call MAR_fnc_log;};
";
FNC_A3_ADMINLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_ADMINLOG:%1 (v0011A)',_this];
	diag_log ('<infiSTAR.de>AdminLog| '+_this+' (v0011A)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de__ADMINLOG'] call MAR_fnc_log;};
";
FNC_A3_CONNECTLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_CONNECTLOG:%1 (v0011A)',_this];
	diag_log ('<infiSTAR.de>ConnectLog| '+_this+' (v0011A)');
";
FNC_A3_RESOLVESTEAMNAMELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_RESOLVESTEAMNAMELOG:%1 (v0011A)',_this];
	diag_log ('<infiSTAR.de>ResolveSteamName| '+_this+' (v0011A)');
...
_A3AHstring = "
	...
	FNC_INFISERVERKICK = compileFinal ""
		...
		diag_log format['<infiSTAR.de>KickLog| %1(%2)   (v0011A)',_name,_uid];
	...
	FNC_A3_infiSTARBAN = {
		...
		if(_reason == '')then
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) (v0011A)',_name,_uid];
			diag_log format['<infiSTAR.de>BanLog| %1(%2)   (v0011A)',_name,_uid];
		}
		else
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) %3 (v0011A)',_name,_uid,_reason];
			diag_log format['<infiSTAR.de>BanLog| %1(%2) %3 (v0011A)',_name,_uid,_reason];
		};
	...
	_FNC_PVAH_AdminReq = {
		...
		if(isNil '_clientNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _clientNetID != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq clientNetID wrong typeName: %1   %2 (v0011A)',typeName _clientNetID,_clientNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_clientNetID == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is empty (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		_ObjFromNetID = objectFromNetId _clientNetID;
		if(isNil '_ObjFromNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _ObjFromNetID != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq ObjFromNetID wrong typeName: %1   %2 (v0011A)',typeName _ObjFromNetID,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _ObjFromNetID)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq ObjFromNetID is Null (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_array')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _array != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq wrong typeName: %1   %2 (v0011A)',typeName _array,_array];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_option')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq option is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _option != 'SCALAR')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq option wrong typeName: %1   %2 (v0011A)',typeName _option,_option];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _playerObj != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj wrong typeName: %1   %2 (v0011A)',typeName _playerObj,_playerObj];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Null (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if!(_ObjFromNetID isEqualTo _playerObj)then
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj != _ObjFromNetID - %1/2  (v0011A)',_playerObj,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		...
		if(!isPlayer _playerObj)then
		{
			if(_option == 1234)then
			{
				...
				if(isNil '_puid')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + 'AdminReq puid is Nil (v0011A)';
					_log call FNC_A3_SURVEILLANCELOG;
				};
				if(typeName _puid != 'STRING')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['AdminReq puid wrong typeName: %1   %2 (v0011A)',typeName _puid,_puid];
					_log call FNC_A3_SURVEILLANCELOG;
				};
				
				_mytime = call fnc_getserverTime;
				_log = format['AdminReq playerObj is not a player   %1(%2 - %3) - trying to resolve player via allPlayers (v0011A)',name _playerObj,getPlayerUID _playerObj,_puid];
				_log = _mytime + _log;
				_log call FNC_A3_SURVEILLANCELOG;
				if(_puid == '')exitWith{};
				{
		...
		if(!isPlayer _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = format['AdminReq playerObj is not a player   %1(%2) (v0011A)',name _playerObj,getPlayerUID _playerObj];
			_log = _mytime + _log;
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq tokenreceived wrong typeName: %1   %2 (v0011A)',typeName _tokenreceived,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_tokenreceived == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is empty (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_clientUID isEqualTo '')then
		{
			...
			if(_foundtokenid > 0)then
			{
				...
				if(!isNil '_puidfound')then
				{
					...
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['%1(%2) | AdminReq - SERVER RESOLVED UID! (v0011A)',_clientName,_clientUID];
					_log call FNC_A3_SURVEILLANCELOG;
				};
			};
		};
		...
		if(!_result)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - BadToken %3 (v0011A)',_clientName,_clientUID,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		_fnc_NoUID = {
			...
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - !!!!!!ADMIN-LOGIN-ERROR!!!!!! (v0011A)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		call {
		...
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - Attempted to Use the AdminMenu (v0011A)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_option == 1234)exitWith
		{
			...
			if(_puid == _clientUID)then
			{
				if!(_clientUID in _locDevs)then{
					diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v0011A)',_clientName,_clientUID];
				};
				"; if(_enableIngameLogs)then{ _A3AHstring = _A3AHstring + "
		...
		if(_option == 0)exitWith
		{
			...
			{
				_vehicleObject = [_vehicleClassName, [0,0,1000], _dir, true, _pinCode] call ExileServer_object_vehicle_createPersistentVehicle;
			}
			else
			{
				_vehicleObject = [_vehicleClassName, [0,0,1000], _dir, true] call ExileServer_object_vehicle_createNonPersistentVehicle;
			};
			if(isNull _vehicleObject)exitWith{};
			
			_position2d = _position;
			_position2d set [2,0];
			...
			if(_vehicleClassName select [0,5] isEqualTo 'Exile')then
			{
				...
				if!(_target isEqualTo _playerObj)then
				{
					[[_vehicleClassName,_pinCode],_code,(owner _playerObj),false] call FN_infiSTAR_S;
				};
				...
				[_vehicleObject, _target]  call ExileServer_system_swapOwnershipQueue_add;
			};
		};
		...
		if(_option == 3)exitWith
		{
			...
			_class = _array select 4;
			
			_object = objNull;
			_wpnh = nearestObjects[_target,['GroundWeaponHolder'],3];
			if!(_wpnh isEqualTo [])then{_object = _wpnh select 0;};
			if(isNull _object)then
			{
				_object = createVehicle ['GroundWeaponHolder',_pos,[],3,'CAN_COLLIDE'];
	...
	_FNC_AH_KICKLOGSPAWN =
	{
		...
		if(isNil '_input')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'FNC_AH_KICKLOGSPAWN _this is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_tokenreceived _this is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_tokenreceived wrong type %1 (v0011A)',typeName _tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_arraysent')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_arraysent _this is Nil (v0011A)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _arraysent != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_arraysent wrong type %1 (v0011A)',typeName _arraysent];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _netId != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_netId wrong type %1 (v0011A)',typeName _netId];
			_log call FNC_A3_SURVEILLANCELOG;
		};
...
_AH_MAIN_BLOCK = {
	...
	if(!isNil'AH_STARTED_ALREADY')exitWith{
		_this spawn {
			...
			[_clientName,_clientUID,'SLOG_SKICK',toArray(_log)] call "+_AHKickLog+";
			(findDisplay 46)closeDisplay 0;
		};
	};
	...
	if!(_clientName isEqualTo _name)then
	{
		...
		[_clientName,_puid,'SLOG_SKICK',toArray(_log)] call "+_AHKickLog+";
		(findDisplay 46)closeDisplay 0;
	};
...
"; }; _A3AHstring = _A3AHstring + "
	_fn_log_glitch = {
		try
		{
			...
			if(_flags isEqualTo [])then{throw false;};
...
"; if(_wall_glitch_object)then{ _A3AHstring = _A3AHstring + "
			_eyePos = eyePos player;
			_lineIntersectsObjs = lineIntersectsObjs [_eyePos, _eyePos];
			{
				if(_x isKindOf 'Exile_Construction_Abstract_Static')then
				{
					...
					if(call _fn_log_glitch)then
					{
						...
						if(time > _glitchReported)then
						{
							...
							_log = format['WALL GLITCH ATTEMPT (attempted breach of %1) - @%2 %3',typeOf _x,mapGridPosition _x,getPosATL _x];
...
"; }; _A3AHstring = _A3AHstring + "
	...
	if(_isNormal)then
	{
		...
		"; if(_UAT)then{ _A3AHstring = _A3AHstring + "
			[_name,_puid] spawn {
				private['_name','_puid','_spawnMarkers','_tpc','_falseStates','_fnc_antitp','_fnc_setBack','_lastpos','_lastPosZero','_lastHeightATL','_log','_lasttime','_difftime','_curpos','_curPosZero','_tmpAHpos','_driver'];
				_name = _this select 0;
				_puid = _this select 1;
	...
	_devLog = format['%1 VERSION: 04102015 infiSTAR.de AHAT (v0011A) - server running: %2:784',VERSIONCHECKRESULT,call _GET_TIME_TIME];diag_log _devLog;
	"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
		...
		if(_puid in "+_adminsA+")then
		{
			...
			if(isNil'fnc_AdminReq')then
			{
				fnc_AdminReq = compileFinal ""
					...
					["+_YourPlayerToken+",_this,netId player] remoteExecCall ['fnc_AdminReq', 2, false];
				"";
			};
...
_DO_THIS_MORE_OFTEN = {
	if(hasInterFace)then
	{
		...
		} forEach ['"+_t1+"','"+_t2+"','"+_t4+"','"+_t5+"','"+_t6+"','"+_t7+"','"+_t8+"','"+_t9+"'];
		{
			if(!isNil _x)then
			{
				_var = missionNamespace getVariable _x;
		...
		} forEach ['"+_RequestToken+"','"+_AH_RunCheckENDVAR+"','"+_AHpos+"'];
		...
		while{1==1}do
		{
			...
			onPlayerConnected {
				...
				missionNamespace setVariable [format['ExileLastCheckedEntityIdFor%1', _owner], 1];
			};
...
_zeroCode = {
	...
	if(hasInterface)then
	{
		...
		"+_TokenCT+" = [_AHKickOFF] spawn {
			...
			waitUntil {"+_t7+"=str diag_tickTime;if(isNil'"+_YourPlayerToken+"')then{"+_YourPlayerToken+"='';};!(str _tmpYourPlayerToken isEqualTo str "+_YourPlayerToken+")};
