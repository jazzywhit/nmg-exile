...
if (_index > -1) then
{
	...
	try 
	{
		...
		switch (_currentContainerType) do
		{
			...
			case 2:
			{	
				if !([uniformContainer player, _itemClassName] call ExileClient_util_containerCargo_canAdd) then
				{
					throw 9;
				};
			};
			case 3:
			{
				if !([vestContainer player, _itemClassName] call ExileClient_util_containerCargo_canAdd) then
				{
					throw 9;
				};
			};
			case 4:
			{
				if !([backpackContainer player, _itemClassName] call ExileClient_util_containerCargo_canAdd) then
				{
					throw 9;
				};
			};
