...
switch (_itemType select 0) do
{
	case "Equipment": 
	{
		switch (_itemType select 1) do
		{
			case "Headgear": 
			{	
				_armor = getNumber(configFile >> "CfgWeapons" >> _itemClassName >> "ItemInfo" >> "armor");
				...
				[
					...
					linearConversion [0, 6, _armor, 0, 1, true]
				];
				_stats pushBack
				[
					"MASS", 
			...
			case "Vest":
			{
				...
				[
					"ARMOR", 
					format["<t size='1' font='puristaMedium' align='right'>%1</t>", _armor],
					linearConversion [0, 100, _armor, 0, 1, true]
				];
				_stats pushBack
				[
					"CAPACITY", 
...
_popTabs = getNumber(missionConfigFile >> "CfgExileArsenal" >> _itemClassName >> "price");
_stats pushBack
[
	"SALES PRICE", 
	format["<t size='1' font='puristaMedium' align='right'>%1<img image='\exile_client\texture\ui\poptab_inline_ca.paa' size='1' shadow='true' /></t>", _popTabs],
	0
];
_stats