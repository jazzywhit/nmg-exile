...
private["_originalVehicleObject","_dialog","_skinListBox","_parentClassName","_vehicleCustomsConfig","_availableSkins","_skinClass","_skinPrice","_indexEntryIndex","_purchaseButton"];
...
if (isClass (missionConfigFile >> "CfgVehicleCustoms" >> _parentClassName) ) then
{
	...
	_availableSkins = getArray(_vehicleCustomsConfig >> "skins");
	{
		...
		_indexEntryIndex = _skinListBox lbAdd getText(configFile >> "CfgVehicles" >> _skinClass >> "skinName");
		_skinListBox lbSetData [_indexEntryIndex, _skinClass];
		if !(ExileClientVehicleCustomsOriginalSkin isEqualTo _skinClass) then
		{
