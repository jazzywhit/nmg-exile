...
private["_display","_vehiclePanelControl","_vehicle","_showVehiclePanel","_showHeight","_showSpeed","_showFuel","_showDriverAmmo","_vehicleRole","_fuelControl","_speedControl","_heightControl","_speed","_height","_vehicleClassName","_fuel","_tankSize","_tankSizeString","_fuelRemaining","_fuelRemainingString"];
...
if (diag_tickTime - ExileHudLastVehicleRenderedAt >= 0.1) then
{
	...
	else 
	{
		...
		if (_showHeight) then
		{
			...
			_heightControl ctrlSetText (format ["%1m", _height]);		
		};
		if (_showFuel) then
		{
