/*
	Strider difficult/hardcore - reworked by [CiC]red_ned http://cic-gaming.co.uk
...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION nedstrider_mission_dh.sqf with invalid parameters: %1",_this];
};
...
_msgStart = ['#FFFF00',"A Strider is parked at a skilled base! Go kill them and steal it"];
...
_msgStart = ['#FFFF00',"A Strider is parked at a hardcore base! Go kill them and steal it"];
_AICount = (10 + (round (random 8)));
_crate_weapons 		= 4;
_crate_items 		= 10;
_crate_backpacks 	= 4;						
								};
