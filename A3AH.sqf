...
FNC_A3_LOG_RAND_VAR = compileFinal "
	'ARMA_LOG' callExtension format['A3_RANDOMVAR_LOG:%1 (v0220)',_this];
	diag_log ('<infiSTAR.de>RandomVarLog| '+_this+' (v0220)');
";
_allRandomGenVars = [];
_counts = [];
{
...
_BIS_fnc_endMission = call _fnc_RandomGen;format['_BIS_fnc_endMission: %1',_BIS_fnc_endMission] call FNC_A3_LOG_RAND_VAR;
_BIS_fnc_param = call _fnc_RandomGen;format['_BIS_fnc_param: %1',_BIS_fnc_param] call FNC_A3_LOG_RAND_VAR;
_BIS_fnc_paramIn = call _fnc_RandomGen;format['_BIS_fnc_paramIn: %1',_BIS_fnc_paramIn] call FNC_A3_LOG_RAND_VAR;
_BIS_fnc_returnChildren = call _fnc_RandomGen;format['_BIS_fnc_returnChildren: %1',_BIS_fnc_returnChildren] call FNC_A3_LOG_RAND_VAR;
...
call compile (""+_BIS_fnc_endMission+" = str BIS_fnc_endMission;publicVariable '"+_BIS_fnc_endMission+"';");
call compile (""+_BIS_fnc_param+" = str BIS_fnc_param;publicVariable '"+_BIS_fnc_param+"';");
call compile (""+_BIS_fnc_paramIn+" = str BIS_fnc_paramIn;publicVariable '"+_BIS_fnc_paramIn+"';");
call compile (""+_BIS_fnc_returnChildren+" = str BIS_fnc_returnChildren;publicVariable '"+_BIS_fnc_returnChildren+"';");
...
[
	...
	'flo_simon','sonicccc_','fury_','phoenix_','_my_new_bullet_man','_jm3','_bullet',
...
FNC_A3_HACKLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_HACKLOG:%1 [FPS: %2|THREADS: %3] (v0220)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>HL| '+_this+' (v0220)');
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_SURVEILLANCELOG:%1 [FPS: %2|THREADS: %3] (v0220)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>SL| '+_this+' (v0220)');
";
FNC_A3_ADMINLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_ADMINLOG:%1 (v0220)',_this];
	diag_log ('<infiSTAR.de>AdminLog| '+_this+' (v0220)');
";
FNC_A3_CONNECTLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_CONNECTLOG:%1 (v0220)',_this];
	diag_log ('<infiSTAR.de>ConnectLog| '+_this+' (v0220)');
";
...
_A3AHstring = "
	...
	FNC_INFISERVERKICK = compileFinal ""
		...
		_name = _this select 0;
		_uid = _this select 1;
		diag_log format['<infiSTAR.de>KickLog| %1(%2)   (v0220)',_name,_uid];
	...
	FNC_A3_infiSTARBAN = {
		...
		if(_reason == '')then
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) (v0220)',_name,_uid];
			diag_log format['<infiSTAR.de>BanLog| %1(%2)   (v0220)',_name,_uid];
		}
		else
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) %3 (v0220)',_name,_uid,_reason];
			diag_log format['<infiSTAR.de>BanLog| %1(%2) %3 (v0220)',_name,_uid,_reason];
		};
	...
	_fnc_infiSTAR_PlayerLog = {
		...
		if(_uid != '')then
		{
			...
			if(_uid in "+str _BlackList+")exitWith{
				_log = 'Has been banned for Hacking before..! (banned thorugh _badpeople array (run.sqf))';
				[_name,_uid,'BAN',toArray(_log)] call "+_FNC_AH373_KICKLOG+";
			};
			...
			if!(_uid in PlayerLogArrayMNS)then
			{
				...
				_log = format['New Player: %1(%2)   (v0220)',_name,_uid];
	...
	_FNC_PVAH_AdminReq = {
		...
		if(isNil '_clientNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is Nil (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _clientNetID != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq clientNetID wrong typeName: %1   %2 (v0220)',typeName _clientNetID,_clientNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_clientNetID == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is empty (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		_ObjFromNetID = objectFromNetId _clientNetID;
		if(isNil '_ObjFromNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Nil (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _ObjFromNetID != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq ObjFromNetID wrong typeName: %1   %2 (v0220)',typeName _ObjFromNetID,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _ObjFromNetID)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq ObjFromNetID is Null (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_array')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq is Nil (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _array != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq wrong typeName: %1   %2 (v0220)',typeName _array,_array];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_option')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq option is Nil (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _option != 'SCALAR')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq option wrong typeName: %1   %2 (v0220)',typeName _option,_option];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _playerObj != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj wrong typeName: %1   %2 (v0220)',typeName _playerObj,_playerObj];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Null (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if!(_ObjFromNetID isEqualTo _playerObj)then
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj != _ObjFromNetID - %1/2  (v0220)',_playerObj,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		...
		if(!isPlayer _playerObj)then
		{
			if(_option == 1234)then
			{
				...
				if(isNil '_puid')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + 'AdminReq puid is Nil (v0220)';
					_log call FNC_A3_SURVEILLANCELOG;
				};
				if(typeName _puid != 'STRING')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['AdminReq puid wrong typeName: %1   %2 (v0220)',typeName _puid,_puid];
					_log call FNC_A3_SURVEILLANCELOG;
				};
				
				_mytime = call fnc_getserverTime;
				_log = format['AdminReq playerObj is not a player   %1(%2 - %3) - trying to resolve player via playableUnits (v0220)',name _playerObj,getPlayerUID _playerObj,_puid];
				_log = _mytime + _log;
				_log call FNC_A3_SURVEILLANCELOG;
				if(_puid == '')exitWith{};
				{
		...
		if(!isPlayer _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = format['AdminReq playerObj is not a player   %1(%2) (v0220)',name _playerObj,getPlayerUID _playerObj];
			_log = _mytime + _log;
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is Nil (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq tokenreceived wrong typeName: %1   %2 (v0220)',typeName _tokenreceived,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_tokenreceived == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is empty (v0220)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_clientUID isEqualTo '')then
		{
			...
			if(_foundtokenid > 0)then
			{
				...
				if(!isNil '_puidfound')then
				{
					...
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['%1(%2) | AdminReq - SERVER RESOLVED UID! (v0220)',_clientName,_clientUID];
					_log call FNC_A3_SURVEILLANCELOG;
				};
			};
		};
		...
		if(!_result)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - BadToken %3 (v0220)',_clientName,_clientUID,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		_fnc_NoUID = {
			...
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - !!!!!!ADMIN-LOGIN-ERROR!!!!!! (v0220)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		call {
		...
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - Attempted to Use the AdminMenu (v0220)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_option == 1234)exitWith
		{
			...
			if(_puid == _clientUID)then
			{
				if!(_clientUID in _locDevs)then{
					diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v0220)',_clientName,_clientUID];
				};
				"; if(_enableIngameLogs)then{ _A3AHstring = _A3AHstring + "
...
_AH_MAIN_BLOCK = {
			...
			"; if((_MOD == 'Epoch')&&(_AWL))then{ _A3AHstring = _A3AHstring + "
				if(player == vehicle player)then
				{
					...
					if((inputAction 'EvasiveLeft' > 0)||(inputAction 'EvasiveRight' > 0)||(_eyeDir select 2 > 0.5)||(_xdif > 0.55))then
					{
						...
						{
							if(_faded)then{_faded=false;TitleText ['!!!\nDO NOT LOOK THROUGH WALLS\n!!!\n\n\n\nAntiHack by infiSTAR.de','PLAIN'];};
						}
						else
						{
							{
		...
		"; if((_UBV)||(_UBF)||(_UBV2))then{ _A3AHstring = _A3AHstring + "
			[_name,_puid,_admins] spawn {
				...
				"; if(_UBV)then{ _A3AHstring = _A3AHstring + "
					...
					_blacklistedVariables append ['"+_AH_MAIN_BLOCK+"','JoinOrNotJoinIsTheQuestion','noRecoilRun','A3FFrun'];
					if(_puid in _admins)then
					{
						{
		...
		[_name,_puid,_admins] spawn {
			...
			while{1==1}do
			{
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					...
					if(EPOCH_playerEnergy > 2500 || EPOCH_playerEnergy < -1000)then
					{
	...
	_devLog = format['<infiSTAR.de> %1 VERSION: 15092015 infiSTAR.de AHAT (v0220) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
	...
	if(_puid in _admins)then
	{
		...
		if(_puid in "+_adminsA+")then
		{
			...
			';
			"+_MAIN_CODE_C+" = nil;
			'"+_MAIN_CODE_C+"' addPublicVariableEventhandler
			{
				_code = call compile (_this select 1);
				"+_MAIN_CODE_C+" = nil;
			};
			...
			admin_d0_MASTER = {
				...
				{
					_mark = createMarker ['respawn_west', '+str _respawn_west+'];
					_mark setMarkerColor 'Default';
					_mark setMarkerText _do;
				}
				else
				{
					'respawn_west' setMarkerText _do;
				};
				switch _opt do {
					case 0:{
...
"+_ZEROZEROZERO+" = (""
	...
	if((!isNil'"+_BIS_fnc_endMission+"')&&(!isNil'BIS_fnc_endMission'))then
	{
		if!("+_BIS_fnc_endMission+" isEqualTo str BIS_fnc_endMission)then
		{
			_log = format['BIS_fnc_endMission has been changed: %1',BIS_fnc_endMission];
			_log = _log select [0,500];
			[profileName,getPlayerUID player,'HLOG_SKICK',toArray(_log)] call _AH373KICKLOG;
			[] spawn _AHKickOFF;
		};
	};
	if((!isNil'"+_BIS_fnc_param+"')&&(!isNil'BIS_fnc_param'))then
	{
		if!("+_BIS_fnc_param+" isEqualTo str BIS_fnc_param)then
		{
	...
	if((!isNil'"+_BIS_fnc_paramIn+"')&&(!isNil'BIS_fnc_paramIn'))then
	{
		if!("+_BIS_fnc_paramIn+" isEqualTo str BIS_fnc_paramIn)then
		{
	...
	if((!isNil'"+_BIS_fnc_returnChildren+"')&&(!isNil'BIS_fnc_returnChildren'))then
	{
		if!("+_BIS_fnc_returnChildren+" isEqualTo str BIS_fnc_returnChildren)then
		{
	...
	{
		_text = ctrlText ((findDisplay 49) displayCtrl 120);
		_ltext = toLower _text;
		if(_ltext find 'g-e-f' != -1)then
		{
			_log = format['BadText: %1 [g-e-f]',_text];
			[profileName,getPlayerUID player,'BAN',toArray(_log)] call _AH373KICKLOG;
			[] spawn _AHKickOFF;
		};
	};
	"; if(_UMH)then{ _A3AHstring = _A3AHstring + "
		[_AH373KICKLOG,_AHKickOFF] spawn {
			_AH373KICKLOG = _this select 0;
			_AHKickOFF = _this select 1;
			...
			_UMH_ARRAYSERVER = ""+str("+str _UMH_ARRAYSERVER+")+"";
			_UMH_ARRAY = ""+str("+str _UMH_ARRAY+")+"";
			{
	...
	[] spawn {
				...
				fnc_infiSTAR_HandleDisconnect = compileFinal '
					...
					if(alive _plyr)then
					{
						...
						if(_inCombat == 1)then
						{
							...
							''ARMA_LOG'' callExtension format[''A3_COMBATLOG:%1 (v0220)'',_log];
							diag_log (''<infiSTAR.de>CombatLog| ''+_log+'' (v0220)'');
		...
		_fnc_zero_one = {
			...
			_zerotwo = (""
				...
				if(isNil'"+_t15+"')then{"+_t15+"=str diag_tickTime;};
			"");
		...
		"; if((_MOD == 'Epoch')&&(_MPH))then{ _A3AHstring = _A3AHstring + "
			LOG_MPHIT_HANDLER =
			{
				...
				if(_log!='')then
				{
					'ARMA_LOG' callExtension format['A3_MPHIT:%1 (v0220)',_log];
				};
			};
		...
		_code = ""
			...
			if(hasInterface)then
			{
				...
				'"+_AH_MAIN_BLOCK+"' addPublicVariableEventhandler compileFinal
				""""
					if(!isNil'AH_STARTED_ALREADY')exitWith{[] spawn {sleep 1;(findDisplay 46)closeDisplay 0;};['epicFail',false,true] call compile (preprocessfile 'A3\functions_f\Misc\fn_endMission.sqf');};
					AH_STARTED_ALREADY = true;
					
					"+_t14+"=str diag_tickTime;
					if(!isNil'"+_AH_MAIN_BLOCK+"')then{if(typeName "+_AH_MAIN_BLOCK+" == 'STRING')then{"+_AH_MAIN_BLOCK+"=nil;};};
		...
		while{1==1}do
		{
			...
			if(time > _timer0)then
			{
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					...
					{
						_this spawn {
							private['_array','_arrayIn','_arrayOut','_plyr','_cIndex','_vars','_current_crypto'];
							_array = _this select 1;
...
call compile ("
	FNC_DO_CLIENT_VAR_DUMP = compileFinal ""
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0220)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0220)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2: Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0220)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0220)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0220)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0220)',_name,_puid];
		};
		...
		[_name,_puid] spawn {
			...
			if(time > _timeOut)exitWith
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP TIMED OUT --- (v0220)',_name,_puid];
				diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP TIMED OUT --- (v0220)',_name,_puid];
			};
			...
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0220)',_name,_puid];
				diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0220)',_name,_puid];
			};
			
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- STARTING DUMP --- (v0220)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- STARTING DUMP --- (v0220)',_name,_puid];
			{
			...
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP ENDED --- (v0220)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP ENDED --- (v0220)',_name,_puid];
		};
	"";
");
